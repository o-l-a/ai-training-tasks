...
class alphaBetaNode(object):
    ...
    def fillWithChildren(self):
        ...
        for field in emptyFields:
            ...
            newChild = alphaBetaNode(newChild)
        ...
        for i in range(depth):
            if depth != 0:
                if i == depth - 1:
                    ...
                    print('               ', end='')
        print('(#', self.id, ', v=', self.v, ', \u03B1=', self.alpha, ', \u03B2=', self.beta, ')', sep='')
    ...
    def maxValue(self, alpha, beta):
        ...
        if self.isAFinalState():
            ...
            self.v = -np.Inf
            for child in self.children:
                self.v = int(np.max([self.v, child.minValue(self.alpha, self.beta)]))
                self.alpha = np.max([self.alpha, self.v])
                # self.printTree()
    ...
    def minValue(self, alpha, beta):
        ...
        if self.isAFinalState():
            ...
            self.v = np.Inf
            for child in self.children:
                self.v = int(np.min([self.v, child.maxValue(self.alpha, self.beta)]))
                self.beta = np.min([self.beta, self.v])
                # self.printTree()
                if self.alpha >= self.beta:
                    break
        print('-' * 60, '\nMIN')
